...
class WildcatTop(file: String, dmemNrByte: Int = 4096) extends Module {
  ...
  tx.io.channel.bits := memArbiter.io.cpu.wrData(7, 0)
  tx.io.channel.valid := false.B
  rx.io.channel.ready := false.B

  val uartStatusReg = RegNext(rx.io.channel.valid ## tx.io.channel.ready)
  val memAddressReg = RegNext(memArbiter.io.cpu.rdAddress)
  ...
  when((cpu.io.dmem.wrAddress(31, 28) === 0xf.U) && cpu.io.dmem.wrEnable(0)) {
    when(
      cpu.io.dmem.wrAddress(19, 16) === 0.U && cpu.io.dmem
        .wrAddress(3, 0) === 4.U
    ) {
    ...
    // Fix this line later
    // memArbiter.io.dmem.wrEnable := VecInit(Seq.fill(4)(false.B))
  }
